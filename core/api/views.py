...
class OrderQuantityUpdateView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if order_qs.exists():
            order = order_qs[0]
            # Проверка наличия продукта заказа, в заказе
            if order.items.filter(item__slug=item.slug).exists():
                ...
                return Response({"message": "Продукта нет в заказе"}, status=HTTP_400_BAD_REQUEST)
        else:
            return Response({"message": "Заказ не найден"}, status=HTTP_400_BAD_REQUEST)
